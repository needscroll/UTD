        ...
        for (BasicBlock& b : fun) {
            ...
            for (Instruction& i : b) {
                ...
                if (fun.getName() == "fib")
                {
                  ...
                  {
                    IRBuilder<> IRB(&i); // new instruction
                    Value * args[2];
                    args[0] = i.getOperand(0);
                    args[1] = ConstantInt::get(IntegerType::get(M.getContext(), 32), 0, false);
                    IRB.CreateCall(M.getFunction("fib_logger"),args);
                  } else if (strcmp(i.getOpcodeName(),"call") == 0 && i.getOperand(1)->getName() == "fib")
                  {
                    IRBuilder<> IRB(&i); // new instruction
                    Value * args[2];
                    args[0] = i.getOperand(0);
                    args[1] = ConstantInt::get(IntegerType::get(M.getContext(), 32), 1, false);
                    IRB.CreateCall(M.getFunction("fib_logger"),args);
                  }
              }
            }
