...
sw	$s4, 4($sp)	#store whether comp or human is playing
sw	$ra, 8($sp)	#store return address
...
sw	$t2,	8($sp)		#save $t2 value
sw	$t3,	12($sp)		#save $t3 value
sw	$t4,	16($sp)		#save $t4 value
sw	$t5,	20($sp)		#save $t5 value
sw	$ra,	24($sp)		#save return address
add	$t0,	$zero,	$gp		#set to start of $gp
ld	$t1,	colorLightGreen		#set some initial values
ld	$t2,	colorDarkGreen
move	$t3,	$zero			#running total value for columns
move	$t4,	$zero			#running total value for rows
li	$t5,	4			#termination value for loops
drawBoardRowLoop:
beq	$t4,	$t5,	drawBoardExit
move	$t3,	$zero			#reset running total value
jal	drawBoardLoop
move	$t3,	$zero			#reset running total value
jal	drawBoardLoop1
addi	$t4,	$t4,	1		#increment running total for rows
j	drawBoardRowLoop		#loop again
drawBoardLoop:
beq	$t3,	$t5,	drawBoardLoopExit	#if running total == 8, break loop
sw 	$t1,	($t0)		#color pixel lightGreen
addi	$t0,	$t0,	4	#increment frameBuffer to next pixel/box
sw	$t2,	($t0)		#color pixel darkGreen
addi	$t0,	$t0,	4	#increment frameBuffer to next pixel/box
addi	$t3,	$t3,	1	#increment running total
j	drawBoardLoop		#loop again
drawBoardLoopExit:
jr	$ra
drawBoardLoop1:
beq	$t3,	$t5,	drawBoardLoop1Exit	#if running total == 8, break loop
sw 	$t2,	($t0)		#color pixel lightGreen
addi	$t0,	$t0,	4	#increment frameBuffer to next pixel/box
...
lw	$t5,	20($sp)		#restore $t5 value
lw	$t4,	16($sp)		#restore $t4 value
lw	$t3,	12($sp)		#restore $t3 value
lw	$t2,	8($sp)		#restore $t2 value
